...
/*	
   ...
   // Assert.
	...
	@Test
	public void getAllPostsTest() throws ParseException
	{
	...
	//	Date date1 = (Date) new SimpleDateFormat("dd/MM/yyyy").parse(sDate1);
		...
		Post postOne = new Post(1, "This is a sample post", 2, sqlDate);
		Post postTwo = new Post(2, "This is another post", 2, sqlDate);
		...
		//test
		List<Post> postList = postRepository.findByUserId(0);

		assertEquals(3, postList.size());
	//	verify(postRepository, times(1)).getPostList();
	}
	...
	//	when(postRepository.findByUserId(1)).thenReturn(new Post(1,"Lokesh",2, sqlDate));
		when(postRepository.findAll()).thenReturn(Collections.singletonList(posts));
		
		assertEquals("Lokesh", post.getFirstName());
		assertEquals("Gupta", post.getLastName());
		assertEquals("user@email.com", post.getEmail());
	}

	@Test
	public void createPostTest()
	{
		PostVO post = new PostVO(1,"Lokesh","Gupta","user@email.com");
